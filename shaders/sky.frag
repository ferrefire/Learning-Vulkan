...
float GetDensity(vec3 point, vec3 center)
{
	...
	float localDensity = exp(-height01 / rayleighHeightScale) * (1.0 - height01);
	return (localDensity);
...
float GetOpticalDepth(vec3 point, vec3 center, vec3 direction, float len)
{
	...
	float stepSize = len * (1.0 / OPTICAL_ITERATIONS);
...
vec3 GetScattering()
{
	...
	vec3 center = variables.viewPosition;
	center.y -= surface;
	...
	vec2 tRange = AtmosphereIntersect(pixelStart, pixelDirection, center);
	...
	float stepSize = tRange.y * (1.0 / SCATTER_ITERATIONS);
	...
	for (int i = 0; i < SCATTER_ITERATIONS; i++)
	{
		...
		float sunLen = AtmosphereIntersect(scatterPosition, variables.lightDirection, center).y;
		float sunOptical = GetOpticalDepth(scatterPosition, center, variables.lightDirection, sunLen);
		viewOptical = GetOpticalDepth(scatterPosition, center, -pixelDirection, i * stepSize);
		vec3 transmittance = exp(-(sunOptical + viewOptical) * rayleighBeta);
		float localDensity = GetDensity(scatterPosition, center);
