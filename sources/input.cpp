...
void Input::SetKeyStatus()
{
    for (auto &key : keys)
    {
        ...
        if (key.second.mouse) key.second.down = glfwGetMouseButton(Manager::GetWindow().data, key.first) == GLFW_PRESS;
        else key.second.down = glfwGetKey(Manager::GetWindow().data, key.first) == GLFW_PRESS;
